...
# coding=utf-8
"""
    使用 global pointer 来完成 xfund 中的 ner 任务
        1. 要关心 layoutlmv3的输出，gp应该只关心文本部分字段;
        2. 对应的gp和原来的gp有些不同，也是因为encoder的多模态输出的缘故;

    参考参数(debug):
        --data_dir /home/ana/data2/datasets/XFUND --language zh --do_train --do_eval
        --model_name_or_path /home/ana/data2/models/layoutlmv3-base-chinese
        --output_dir output.gp.ddp --segment_level_layout 1 --visual_embed 1 --input_size 224
        --max_steps 20 --save_steps 5 --evaluation_strategy steps --eval_steps 10 --logging_steps 5
        --learning_rate 7e-5 --per_device_train_batch_size 2 --gradient_accumulation_steps 2
...
from libs.datasets.xfund_dataset_gp import xfund_dataset_gp, XFund_label2ids_gp
...
def main():
    ...
    if training_args.do_train:
        train_dataset = xfund_dataset_gp(data_args, tokenizer, 'train')
        logger.info(f"train_dataset loaded: len: {len(train_dataset)}")
    if training_args.do_eval:
        eval_dataset = xfund_dataset_gp(data_args, tokenizer, 'eval')
        logger.info(f"eval_dataset loaded: len: {len(eval_dataset)}")
    if training_args.do_predict:
        test_dataset = xfund_dataset_gp(data_args, tokenizer, 'eval')
        logger.info(f"eval_dataset loaded: len: {len(eval_dataset)}")
    ...
    model = GlobalPointerNerModel(encoder, config, len(XFund_label2ids_gp), fixed_text_len=512, RoPE=True, RoPE_dim=2)
    ...
    if last_checkpoint is not None:
        ...
        # model = GlobalPointer(encoder, config, len(train_dataset.label2ids), fixed_text_len=512, RoPE=False, RoPE_dim=2)
    ...
    data_collator = DataCollatorForKeyValueExtractionGp(
        ...
        padding=padding,
        max_length=512,
        num_labels=len(XFund_label2ids_gp),
        fixed_text_length=512
    )

    def get_label_list():
        label_list = [[key, val] for key, val in XFund_label2ids_gp.items()]

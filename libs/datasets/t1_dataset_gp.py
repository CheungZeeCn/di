"""
    针对 task 1 的数据处理函数
    todo: 将整页文本随机选取不同位置的连续片段 来训练跨区域的kv关系的识别
            预测的时候 就可以做到 很多key 放在前面 然后把 要识别的value 放在后面，去判断是否有关系，且不依赖他们是连续的
"""
...
class t1_dataset_gp(Dataset):
    ...
    def __init__(
        ...
        if json_file is None:
            json_file = os.path.join(args.data_dir, "{}.json".format(mode))
    ...
    def load_data(
        ...
        for i in range(num_records):
            ...
            if real_ocr_data is not None and file_id in real_ocr_data:
                ...
                # 可以兼容没有paddle ocr 去模拟真实 ocr结果的场景
                i_real_ocr_data = None

            # 预处理, 处理好span和bbox信息，处理好 关系信息
            ...
            # token tags 相对麻烦一些; 不过当前的任务是没有重叠的(todo: 有重叠的要换一种记录方式)
            ...
            for j in range(len(i_merged_bbox_info)):
                ...
                cur_doc_bboxes.append(box_norm(pick_2_points(cur_item['coord']), width=width, height=height, norm_to=1000))
        ...
        for i in range(len(total_data['lines'])):
            ...
            # 头头映射 + 尾尾映射 所以关系类型 * 2, 详细请参考 Global linker.
        ...
        # DONE; 这里做关系处理 的截断 会 比较复杂一些; total_span_rel_labels 里面存放着我们关心的关系信息;  i, i+num_rel_types 为对应关系的头和尾
        # todo: 目前只有按顺序截断，后续可以随机选bbox 片段，这样模型对业务跨度较长的kv关系也有得到训练
        ...
        for i in range(len(total_input_ids)):
            ...
            while start < len(total_input_ids[i]):
                ...
                # 实际上会比较少用bbox级别的label, 因为颗粒度和线上不一致
        ...
        # todo_DONE: 考虑截断处不要在一个bbox中间;
        # 1. 长文本截断分片，无overlap(这部分在 NER 应该影响不大，但是关系抽取可能会有一定影响);
        # 2. gp_labels 列表(batch_size)的列表(single record)的列表(i,j,label), [ [i, j, lable_id], [i,j,label_id]]
        #
        # 先看span level的效果?
        res = {
            'input_ids': input_ids,
...
class t1_test_dataset_gp(t1_dataset_gp):
    """
        ner 和关系抽取的 dataset
        test 时候用
    """
